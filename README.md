...
Requires **Python 3.10+** (uses `X | None` union type syntax at
runtime in a couple of files — anything older raises a `TypeError` on
import). Raspberry Pi OS Bookworm ships 3.11, fine; Bullseye's 3.9 is
not.

Venv binary path differs by OS — `.venv/Scripts/...` on Windows,
`.venv/bin/...` on Linux/macOS. Examples below use the Linux path
(what a CM4/Pi target actually needs); swap in `Scripts` if you're
...
npm install                # tailwindcss + htmx.org (dev-time only, not needed on the deploy target)
...
the two commands above after editing templates or `tailwind.config.js`. **Node/npm
is a dev-time tool only** — a deploy target (CM4 included) never needs it, since
those two output files are already committed.
...
```bash
.venv/bin/python -m uvicorn demo_launch:app --host 127.0.0.1 --port 8443
```
...
```bash
.venv/bin/python -m scripts.bootstrap_superadmin --username <you>
```
